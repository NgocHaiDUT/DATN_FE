...
export class AuthController {
  ...
  async register(@Body() registerDto: RegisterDto) {
    if (
      ...
      throw new BadRequestException(ResponseMessages.AUTH.MISSING_VALUE);
    }
  ...
  async login(@Body() loginDto: LoginDto) {
    if (!loginDto || !loginDto.email || !loginDto.password) {
      throw new BadRequestException(
        ResponseMessages.AUTH.EMAIL_PASSWORD_REQUIRED,
      );
    }
  ...
  @ApiResponse({
    status: 200,
    description:
      'Device verified successfully. Returns access token and refresh token.',
    schema: {
      example: {
        success: true,
        access_token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...',
  ...
  @ApiResponse({
    status: 401,
    description: 'Unauthorized - OTP expired or incorrect',
  })
  ...
  async forgotPassword(@Body() forgotPasswordDto: ForgotPasswordDto) {
    if (!forgotPasswordDto || !forgotPasswordDto.email) {
      throw new BadRequestException(ResponseMessages.AUTH.EMAIL_REQUIRED);
    }
    const user = this.authService.existuser(forgotPasswordDto.email);
    if (!user) {
      return { success: false, message: ResponseMessages.AUTH.EMAIL_NOT_FOUND };
    }
    ...
    try {
      ...
      return {
        success: false,
        message: ResponseMessages.AUTH.EMAIL_SEND_FAILED,
      };
    }
    ...
    return {
      success: true,
      message: ResponseMessages.AUTH.NEW_PASSWORD_SENT,
    };
  }
  ...
  @ApiResponse({ status: 404, description: ResponseMessages.USER.NOT_FOUND })
  async changePassword(@Body() changePasswordDto: ChangePasswordDto) {
    if (
      ...
      throw new BadRequestException(
        ResponseMessages.AUTH.MISSING_REQUIRED_FIELDS_CHANGE_PASS,
      );
    }
  ...
  async changePasswordFirstTime(
    ...
    if (
      ...
      throw new BadRequestException(
        ResponseMessages.AUTH.MISSING_REQUIRED_FIELDS_CHANGE_PASS_FIRST,
      );
    }
  ...
  async googleCallback(@Req() req, @Res() res: Response) {
    ...
    // Extract device info from OAuth state
    const state = req.query.state as string;
    let deviceType = 'mobile';
    let deviceId: string | undefined = undefined;

    let redirectOrigin: string | undefined = undefined;
    ...
    if (state) {
      try {
        ...
        redirectOrigin = decodedState.redirect_origin || undefined;
      } catch {
        // State parse failure is non-fatal; proceed without device info
      }
    }
    ...
    // Use redirect_origin from OAuth state if available and valid, otherwise fall back to env
    let redirectBase: string;
    if (deviceType === 'web') {
      const allowedPattern =
        /^https?:\/\/(localhost|127\.\d+\.\d+\.\d+|192\.168\.\d+\.\d+|10\.\d+\.\d+\.\d+|pbl6-fe\.duckdns\.org|pbl6-admin\.duckdns\.org)(:\d+)?$/;
      redirectBase =
        redirectOrigin && allowedPattern.test(redirectOrigin)
          ? redirectOrigin
          : process.env.FRONTEND_URL || 'http://localhost:3001';
    } else {
      redirectBase = process.env.MOBILE_URL || 'http://localhost:3001';
    }
  ...
  async refreshToken(@Body() dto: RefreshAccessTokenDto) {
    if (!dto.token || !dto.device_id) {
      throw new BadRequestException(
        ResponseMessages.AUTH.MISSING_REFRESH_TOKEN_OR_DEVICE_ID,
      );
    }
  ...
  async exchangeToken(@Body() dto: ExchangeTokenDto) {
    if (!dto.code || !dto.device_id) {
      throw new BadRequestException(
        ResponseMessages.AUTH.MISSING_CODE_OR_DEVICE_ID,
      );
    }
