...
export class AuthService {
  ...
  async login(dto: LoginDto) {
    ...
    if (
      ...
      process.env.SKIP_DEVICE_OTP === 'true'
    ) {
  ...
  async adminLogin(dto: LoginDto) {
    ...
    if (roleName !== 'admin') {
      throw new UnauthorizedException(
        ResponseMessages.AUTH.ADMIN_ACCESS_REQUIRED,
      );
    }
  ...
  async verifyDevice(dto: VerifyDeviceOtpDto) {
    ...
    if (!latestOtp) {
      throw new UnauthorizedException(ResponseMessages.AUTH.OTP_EXPIRED);
    }

    const isValidOtp = await bcrypt.compare(otp, latestOtp.otp_hash);
    if (!isValidOtp) {
      throw new UnauthorizedException(ResponseMessages.AUTH.INVALID_OTP);
    }
  ...
  private async issueTokens(user: any, device_id: string, device_name: string) {
    ...
    try {
      ...
      console.error(
        '[Auth] Error saving token with upsert, trying delete+create approach:',
        error,
      );
  ...
  async validateUser(email: string, password: string) {
    ...
    if (!user.password_hash) {
      throw new UnauthorizedException(ResponseMessages.AUTH.PASSWORD_NOT_SET);
    }
  ...
  private generateOAuthCode(): string {
    return crypto.randomBytes(16).toString('hex'); // 32-char hex, collision-safe
  }

  async createOAuthCode(dto: CreateOAuthCodeDto) {
    if (!dto.device_id) {
      throw new BadRequestException(ResponseMessages.AUTH.DEVICE_ID_REQUIRED);
    }

    const code = this.generateOAuthCode();
  ...
  async exchangeToken(dto: ExchangeTokenDto) {
    ...
    try {
      // Find OAuth code by code value and expiry only.
      // device_id matching is relaxed: it was optional during Google OAuth code creation.
      ...
      if (!oauthCode) {
        throw new UnauthorizedException({
          ...
          message: ResponseMessages.AUTH.OAUTH_CODE_INVALID,
        });
      }

      // If a device_id was stored with the code, verify it matches
      if (oauthCode.device_id !== null && oauthCode.device_id !== device_id) {
        throw new UnauthorizedException({
          code: ERROR_CODE.OAUTH_CODE_INVALID,
          message: ResponseMessages.AUTH.DEVICE_ID_MISMATCH,
        });
      }
  ...
  async logout(userId: number, deviceId: string, all: boolean) {
    if (all) {
      ...
      return {
        success: true,
        message: `Đã logout tất cả thiết bị (${result.count} devices)`,
      };
    }
