...
export class MessagesService {
  ...
  async getUserConversations(userId: number, queryDto: QueryConversationsDto) {
    ...
    const sortedConversations = allConversations.sort((a, b) => {
      const aTime =
        a.messages[0]?.created_at?.getTime() || a.created_at.getTime();
      const bTime =
        b.messages[0]?.created_at?.getTime() || b.created_at.getTime();
  ...
  async sendMessage(
    ...
    if (!finalConversationId) {
      throw new BadRequestException(
        'Either conversation_id or both sender_id and receiver_id must be provided',
      );
    }
      ...
      if (!shop) {
        ...
        if (!shopStaff) {
          throw new ForbiddenException(
            'You do not have permission to send messages as this shop',
          );
        }
      }
    ...
    console.log('📝 [sendMessage] Sender info:', {
      senderType,
      senderId,
      shopId,
      userId,
    ...
    if (payload && (payload.mediaUrl || payload.mediaFiles)) {
      console.log(
        '📎 [sendMessage] Creating message_media records from payload',
      );
      ...
      console.log(
        '✅ [sendMessage] Created message_media records for message:',
        message.id,
      );
    }
    ...
    if (this.chatGateway?.server) {
      try {
        ...
        if (conversation) {
          ...
          const formattedMessage = {
            ...
            receiverId: shopId
              ...
              : receiver_id ||
                conversation.participants.find(
                  (p) => p.user_id && p.user_id !== (sender_id || userId),
                )?.user_id,
            receiver_id: shopId
              ? conversation.participants.find(
                  (p) => p.user_id && p.user_id !== userId,
          ...
          conversation.participants.forEach((participant) => {
            if (participant.user_id) {
              ...
              console.log(
                `✅ [MessagesService] Emitted newMessage to user ${participant.user_id}`,
              );
            }
            if (participant.shop_id) {
              this.chatGateway.server
                ...
                .emit('newMessage', formattedMessage);
              console.log(
                `✅ [MessagesService] Emitted newMessage to shop ${participant.shop_id}`,
              );
            }
          });
        }
        ...
        console.error(
          '❌ [MessagesService] Failed to emit WebSocket event:',
          error,
        );
      ...
      console.warn(
        '⚠️ [MessagesService] ChatGateway server not available, skipping real-time notification',
      );
    }
  ...
  async getShopMessages(
    ...
    if (!conversation) {
      throw new NotFoundException(
        'Conversation not found or does not belong to this shop',
      );
    }
  ...
  async getUnreadMessagesCount(
    ...
    return this.prisma.messages.count({
      where: {
        ...
        OR: [{ sender_id: { not: userId } }, { sender_id: null }],
        NOT: {
          message_reads: {
            some: {
              user_id: userId,
