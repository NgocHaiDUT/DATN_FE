...
export class AddressService {
  ...
  private async resolveGHNLocationNames(
    ...
    try {
      ...
      throw new BadRequestException(
        ResponseMessages.ADDRESS.GHN_VALIDATION_FAILED,
      );
    }
  ...
  async updateUserAddress(
    ...
    if (!existingAddress) {
      throw new NotFoundException(ResponseMessages.ADDRESS.NOT_FOUND);
    }

    if (existingAddress.user_id !== userId) {
      throw new ForbiddenException(ResponseMessages.ADDRESS.FORBIDDEN_UPDATE);
    }
  ...
  async deleteUserAddress(addressId: number, userId: number) {
    ...
    if (existingAddress.user_id !== userId) {
      throw new ForbiddenException(ResponseMessages.ADDRESS.FORBIDDEN_DELETE);
    }
  ...
  async getUserAddresses(userId: number) {
    ...
    return {
      message: ResponseMessages.ADDRESS.FETCH_SUCCESS,
      data: addresses,
    };
  }
  ...
  async getShopAddresses(shopId: number, userId?: number) {
    if (userId) {
      const shop = await this.prisma.shops.findUnique({
        where: { id: shopId },
        include: {
          shop_staffs: { where: { user_id: userId } },
        },
      });
      if (!shop) {
        throw new NotFoundException('Shop không tồn tại');
      ...
      const isOwner = shop.owner_id === userId;
      const isStaff = shop.shop_staffs.length > 0;
      if (!isOwner && !isStaff) {
        throw new ForbiddenException('Bạn không có quyền xem địa chỉ shop này');
      }
    }

    const shopAddresses = await this.prisma.shop_addresses.findMany({
