...
import { Prisma, product_moderation_status } from '@prisma/client';
@Injectable()
export class ProductService {
  ...
  private async productHasOpenOrders(
    productId: number,
    variantIds: number[] = [],
  ) {
  ...
  async addbrand(userid: number, name: string, slug: string, logo_url: string) {
    ...
    if (ishasbrandname) {
      return {
        success: false,
        message: ResponseMessages.PRODUCT.BRAND_NAME_EXISTS,
      };
    }
    ...
    if (ishasbrandslug) {
      return {
        success: false,
        message: ResponseMessages.PRODUCT.BRAND_SLUG_EXISTS,
      };
    }
    ...
    return {
      success: true,
      message: ResponseMessages.PRODUCT.BRAND_CREATE_SUCCESS,
    };
  }
  ...
  async editbrandname(userid: number, id: number, name: string) {
    ...
    if (!ishasbrand) {
      return {
        success: false,
        message: ResponseMessages.PRODUCT.BRAND_NOT_FOUND,
      };
    }
    ...
    return {
      success: true,
      message: ResponseMessages.PRODUCT.BRAND_UPDATE_SUCCESS,
    };
  }
  ...
  async editbrandslug(userid: number, id: number, slug: string) {
    ...
    return {
      success: true,
      message: ResponseMessages.PRODUCT.BRAND_SLUG_UPDATE_SUCCESS,
    };
  }
  ...
  async editbrandslogo(userid: number, id: number, logo_url: string) {
    ...
    return {
      success: true,
      message: ResponseMessages.PRODUCT.BRAND_LOGO_UPDATE_SUCCESS,
    };
  }
  ...
  async deleteBrand(userid: number, id: number) {
    ...
    if (productsUsingBrand) {
      return {
        success: false,
        message: ResponseMessages.PRODUCT.BRAND_IN_USE,
      };
    }
    ...
    return {
      success: true,
      message: ResponseMessages.PRODUCT.BRAND_DELETE_SUCCESS,
    };
  }
  ...
  async addcategory(
    ...
    if (ishascategoryname) {
      return {
        success: false,
        message: ResponseMessages.PRODUCT.CATEGORY_NAME_EXISTS,
      };
    }
    ...
    if (ishascategoryslug) {
      return {
        success: false,
        message: ResponseMessages.PRODUCT.CATEGORY_SLUG_EXISTS,
      };
    }
    ...
    return {
      success: true,
      message: ResponseMessages.PRODUCT.CATEGORY_CREATE_SUCCESS,
    };
  }
  ...
  async editnamecategory(userid: number, id: number, name: string) {
    ...
    if (!ishascategory)
      return {
        success: false,
        message: ResponseMessages.PRODUCT.CATEGORY_NOT_FOUND,
      };
    ...
    return {
      success: true,
      message: ResponseMessages.PRODUCT.CATEGORY_NAME_UPDATE_SUCCESS,
    };
  }
  ...
  async deleteCategory(userid: number, id: number) {
    ...
    const productsUsingCategory =
      await this.prisma.product_categories.findFirst({
  ...
  async editProduct(
    ...
    try {
      ...
      if (is_published === false && product.is_published) {
        const variantIds = product.product_variants.map(
          (variant) => variant.id,
        );
        const hasOpenOrders = await this.productHasOpenOrders(
          product_id,
          variantIds,
        );
        ...
        if (hasOpenOrders) {
          return {
            success: false,
            message:
              'Không thể ẩn sản phẩm đang có đơn hàng chưa hoàn tất.',
          };
        }
      }
  ...
  async deleteProduct(product_id: number, user_id: number) {
    try {
      ...
      if (product.is_deleted) {
        return { success: false, message: 'Sản phẩm đã được xóa trước đó' };
      }

      // Admin can delete any product. Shop owners can delete their own
      // products. Shop staff must belong to the shop and have manage_product.
      ...
      const hasManageProductPermission =
        permissionNames.has('manage_product');

      if (
        !isAdmin &&
        !isOwner &&
        (!isStaff || !hasManageProductPermission)
      ) {
        return {
          success: false,
      ...
      // Do not delete products that are already referenced by orders.
      ...
      if (hasOpenOrders) {
        return {
          success: false,
          message:
            'Không thể xóa sản phẩm đang có đơn hàng chưa hoàn tất.',
        };
      }

      const deletedAt = new Date();

      // Soft-delete so admins can restore it from Deleted Data management.
      ...
      return {
        success: true,
        message: 'Đã xóa sản phẩm. Bạn có thể khôi phục trong mục dữ liệu đã xoá.',
      };
    } catch (error: any) {
  ...
  async requestProductRestore(
    ...
    try {
      ...
      if (!product.is_deleted) {
        return { success: false, message: 'Sản phẩm này chưa bị xoá' };
      }
      ...
      if (
        ...
        return {
          success: false,
          message: 'Bạn không có quyền yêu cầu khôi phục sản phẩm này',
        };
      }

      const recentRequests = await this.prisma.moderation_logs.findMany({
        where: {
          target_type: 'product',
      ...
      const hasPendingRequest = recentRequests.some((request) => {
        const payload = this.parseRestoreRequestReason(request.reason);
      ...
      if (hasPendingRequest) {
        return {
          success: false,
          message: 'Sản phẩm này đã có yêu cầu khôi phục đang chờ admin duyệt',
        };
      }
      ...
      return {
        success: true,
        message: 'Đã gửi yêu cầu khôi phục sản phẩm đến admin',
      };
    } catch (error: any) {
      console.error('Error requesting product restore:', error);
      return {
        success: false,
        message: 'Lỗi khi gửi yêu cầu khôi phục sản phẩm',
        error: error.message,
      };
    }
  }
  ...
  async getPendingProductDetails(product_id: number, user_id: number) {
    try {
      const moderator = await this.prisma.users.findUnique({
        where: { id: user_id },
        include: {
          role: {
      ...
      const permissions = new Set([
        ...(moderator?.role?.rolePermissions?.map((rp) => rp.permission.name) ||
          []),
        ...(moderator?.userPermissions?.map((up) => up.permission.name) || []),
      ]);
      if (
        moderator?.role?.name !== 'admin' &&
        !permissions.has('manage_shop_admin') &&
        !permissions.has('manage_users')
      ) {
        return {
          success: false,
          message: 'Bạn không có quyền kiểm duyệt sản phẩm',
        };
      }
  ...
  async approveProduct(product_id: number, user_id: number) {
    try {
      ...
      if (
        ...
        return { success: false, message: 'Bạn không có quyền duyệt sản phẩm' };
      }
  ...
  async getAllProductsManager(query: any, currentUser?: any) {
    try {
      const {
        page = 1,
        limit = 12,
      ...
      const isAdminScope =
        ...
        currentUser?.permissions?.includes('manage_shop_admin') ||
      ...
      if (!isAdminScope) {
        if (!where.shop_id) {
          return {
            success: false,
            message: 'Seller must provide shop_id to manage products',
          };
        }

        const shop = await this.prisma.shops.findUnique({
          where: { id: where.shop_id },
          select: { owner_id: true },
        });
        const isOwner = shop?.owner_id === currentUser?.userId;
        const isStaff = await this.prisma.shop_staffs.findFirst({
          where: { shop_id: where.shop_id, user_id: currentUser?.userId },
        });
        if (!isOwner && !isStaff) {
          return {
            success: false,
            message: 'Bạn không có quyền quản lý sản phẩm của shop này',
          };
        }
      }
      ...
      return {
        success: false,
        message: 'Lỗi khi tải sản phẩm dành cho quản lý',
      };
    }
  }
  ...
  async getProductById(id: number) {
    try {
      ...
      if (!product) {
        return {
          success: false,
          message: 'Sản phẩm không tồn tại hoặc chưa được duyệt',
        };
      }

      const sold_count = await this.prisma.order_items.count({
        where: {
          product_id: productId,
      ...
      return {
        success: true,
        product: { ...this.normalizeProduct(product), sold_count },
      };
    } catch (error: any) {
  ...
  async addToCart(
    ...
    try {
      ...
      if (
        ...
        return {
          success: false,
          message: 'Sản phẩm hiện tại không còn khả dụng',
        };
      }
      ...
      if (variant.is_deleted || !variant.is_active) {
        return {
          success: false,
          message: 'Biến thể sản phẩm hiện tại không còn khả dụng',
        };
      }
