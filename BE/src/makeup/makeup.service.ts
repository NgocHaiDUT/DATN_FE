...
export class MakeupService {
  ...
  async recommendProductsBySkintone(skintone: string) {
    ...
    for (const [productType, cats] of Object.entries(this.categoryMap) as Array<
      [keyof typeof this.categoryMap, string[]]
    >) {
      if (!skinColors[productType]) continue;
      ...
      for (const product of products) {
        ...
        const belongs = product.product_categories.some((pc) =>
        ...
        const targetColor = skinColors[productType].color;
        const targetOpacity = skinColors[productType].alpha;
        ...
        if (productMinScore < bestDistance) {
          ...
          bestProduct = {
            ...
            bestMatch: `${productType}: ${skinColors[productType].name}`,
          };
        }
      }
  ...
  async getMakeupProductsByMyShop(userId: number, limitPerCategory = 10) {
    ...
    if (!shopId) {
      throw new NotFoundException(ResponseMessages.MAKEUP.USER_NOT_SHOP_OWNER);
    }
  ...
  async updateVariantShade(
    ...
    if (!variant) {
      throw new NotFoundException(ResponseMessages.MAKEUP.VARIANT_NOT_FOUND);
    }
    ...
    if (!isOwner && !isStaff) {
      throw new ForbiddenException(
        ResponseMessages.MAKEUP.FORBIDDEN_UPDATE_VARIANT,
      );
    }
  ...
  async recordTryOn(userId: number, productId: number, variantId?: number) {
    const session = await this.prisma.tryon_sessions.create({
      data: { user_id: userId, device: 'web', input_type: 'camera' },
    });
    await this.prisma.tryon_items.create({
      data: {
  ...
  async getVrReviewPromptStatus(userId: number) {
    ...
    if (setting?.snooze_until && setting.snooze_until > now) {
      return {
        should_show: false,
        reason: 'snoozed',
        snooze_until: setting.snooze_until,
        next_available_at: setting.snooze_until,
      };
    }
  ...
  async submitVrModelReview(
    ...
    const review = await this.prisma.vr_model_reviews.create({
      data: {
        user_id: userId,
        rating,
    ...
    await this.prisma.vr_review_prompt_settings.upsert({
      where: { user_id: userId },
      create: { user_id: userId, snooze_until: null },
  ...
  async snoozeVrReviewPrompt(userId: number, days?: number, minutes?: number) {
    ...
    return { success: true, snooze_until: setting.snooze_until };
  }

  async getVrReviewAdminStats() {
    const reviews = await this.prisma.vr_model_reviews.findMany({
      orderBy: { created_at: 'desc' },
      take: 200,
      include: {
    ...
    const items = reviews.map((review) => {
      ...
      if (review.content) {
        try {
          parsedContent = JSON.parse(review.content) as typeof parsedContent;
        } catch {
          parsedContent = { note: review.content };
        }
      }

      const source = parsedContent.source || 'prompt';
      sourceCounts[source] = (sourceCounts[source] || 0) + 1;

      Object.entries(parsedContent.aspects || {}).forEach(([aspect, value]) => {
        if (!aspectCounts[aspect]) aspectCounts[aspect] = {};
        aspectCounts[aspect][value] = (aspectCounts[aspect][value] || 0) + 1;
      });

      (parsedContent.suggestions || []).forEach((suggestion) => {
    ...
    return {
      ...
      recent_reviews: items.slice(0, 50),
    };
  }
}
