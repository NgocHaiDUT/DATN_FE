...
export class ReviewsService {
  ...
  async createReview(createReviewDto: CreateReviewDto, userId: number) {
    const { product_id, rating, title, content, media_url } = createReviewDto;
    ...
    try {
      ...
      const verifiedOrder = await this.prisma.orders.findFirst({
        where: {
          user_id,
          status: 'delivered',
          order_items: {
    ...
    return {
      ...
      data: {
        avg_rating: product.avg_rating
          ? parseFloat(product.avg_rating.toString())
          : 0,
