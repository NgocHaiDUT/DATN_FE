...
export class GHNDeliveryService extends DeliveryService {
  ...
  private async makeRequest(
    ...
    try {
      ...
      if (response.data.code !== 200) {
        throw new HttpException(
          response.data.message || 'GHN API error',
      ...
      console.error(
        `GHN API request failed for ${url}:`,
        error.response?.data || error.message,
      );
      throw new HttpException(
        error.response?.data?.message || 'Failed to communicate with GHN API',
        HttpStatus.BAD_GATEWAY,
      );
    }
  }
  ...
  async calculateShippingFee(data: CalculateFeeDto) {
    ...
    if (data.service_type_id === 5) {
      if (!data.items || data.items.length === 0) {
        throw new HttpException(
          'Items are required for Heavy goods service_type_id 5',
      ...
      for (const item of data.items) {
        if (!item.length || !item.width || !item.height || !item.weight) {
          throw new HttpException(
            'Each item must have length, width, height, and weight for Heavy goods service_type_id 5',
            HttpStatus.BAD_REQUEST,
          );
        }
      }
    }
    const extraHeaders = data.shop_id
      ? { ShopId: data.shop_id.toString() }
      : {};
    const { shop_id, ...payload } = data;
    return this.makeRequest(
  ...
  async getShippingOrderDetailByClientCode(clientOrderCode: string) {
    return this.makeRequest('v2/shipping-order/detail-by-client-code', 'post', {
  ...
  async updateShippingOrder(
    ...
    return this.makeRequest(
      ...
      'post',
      { ...updateData, order_code: orderCode },
      headers,
    );
  }
  ...
  async getPrintToken(orderCodes: string[]) {
    return this.makeRequest('v2/a5/gen-token', 'post', {
      order_codes: orderCodes,
    });
  }
}
