...
export class ShopController {
  ...
  @ApiOperation({
    ...
    description:
      'Get all products from a shop including unpublished and pending approval products. Shop owner, authorized staff, or admin can access this endpoint.',
  })
  @ApiParam({
    name: 'shopId',
    description: 'Shop ID',
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - Not shop owner, authorized staff, or admin',
  })
  @ApiResponse({
    status: 404,
    description: 'Shop not found',
  ...
  @Put('commission-rate/all')
  @UseGuards(JwtAuthGuard, PermissionsGuard)
  @RequirePermissions(Permission.MANAGE_SHOP_ADMIN)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({
    summary: 'Update platform commission rate for all shops',
    description:
      'Admin-only endpoint to update the commission rate used when settling future completed orders for every shop. Example: 0.03 = 3%.',
  })
  @ApiBody({ type: UpdateCommissionRateDto })
  @ApiResponse({
    status: 200,
    description: 'Commission rate updated successfully for all shops',
  })
  @ApiResponse({
    status: 400,
    description: 'Bad request - commission_rate must be between 0 and 1',
  })
  @ApiResponse({
    status: 401,
    description: 'Unauthorized - Not logged in',
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - Only admin can update commission rate',
  })
  async updateAllCommissionRates(
    @Body() body: UpdateCommissionRateDto,
    @Req() req: any,
  ) {
    const userId = req.user.userId;
    return this.shopService.updateAllCommissionRates(
      userId,
      body.commission_rate,
    );
  }

  @Put(':shopid/commission-rate')
  @UseGuards(JwtAuthGuard, PermissionsGuard)
  @RequirePermissions(Permission.MANAGE_SHOP_ADMIN)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({
    summary: 'Update platform commission rate for a shop',
    description:
      'Admin-only endpoint to update the commission rate used when settling future completed orders for this shop. Example: 0.03 = 3%.',
  })
  @ApiParam({
    name: 'shopid',
  ...
  async updateCommissionRate(
    ...
    const userId = req.user.userId;
    return this.shopService.updateCommissionRate(
      userId,
      shopid,
      body.commission_rate,
    );
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        message: {
          type: 'string',
          example: 'Shop "Beauty Store" đã được unban (is_verified = true)',
        },
      },
    },
  })
