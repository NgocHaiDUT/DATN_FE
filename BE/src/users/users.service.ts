...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto): Promise<UserEntity> {
    ...
    if (existingUser) {
      throw new BadRequestException(ResponseMessages.AUTH.EMAIL_ALREADY_EXISTS);
    }
  ...
  private async getUserOrThrow(id: number): Promise<users> {
    ...
    if (!user) {
      throw new NotFoundException(ResponseMessages.USER.NOT_FOUND);
    }
    return user;
  }
  ...
  async update(id: number, updateUserDto: UpdateUserDto): Promise<UserEntity> {
    ...
    if (updateUserDto.firstlogin !== undefined)
      data.firstlogin = updateUserDto.firstlogin;
    if (updateUserDto.role_id !== undefined)
      data.role_id = updateUserDto.role_id;
  ...
  async updateAvatar(id: number, avatarUrl: string): Promise<any> {
    ...
    return {
      success: true,
      message: ResponseMessages.USER.UPDATED_SUCCESS,
  ...
  async setUserRole(userId: number, roleId: number): Promise<any> {
    ...
    if (!role) {
      throw new NotFoundException(ResponseMessages.ROLE.NOT_FOUND);
    }
    ...
    return {
      success: true,
      message: ResponseMessages.USER.ROLE_UPDATED_SUCCESS,
      data: this.sanitizeUser(user),
    };
  }
  ...
  async setUserPermissions(
    ...
    if (permissions.length !== permissionIds.length) {
      throw new BadRequestException(ResponseMessages.ROLE.PERMISSION_NOT_FOUND);
    }
    ...
    return {
      success: true,
      message: ResponseMessages.USER.PERMISSIONS_UPDATED_SUCCESS,
      data: {
        user_id: userId,
  ...
  async createPermission(name: string): Promise<any> {
    ...
    if (existingPermission) {
      throw new BadRequestException(
        ResponseMessages.ROLE.PERMISSION_ALREADY_EXISTS,
      );
    }
    ...
    return {
      success: true,
      message: ResponseMessages.ROLE.PERMISSION_CREATED_SUCCESS,
      data: permission,
    };
  }
  ...
  async setRolePermissions(
    ...
    return {
      success: true,
      message: ResponseMessages.ROLE.PERMISSIONS_UPDATED_SUCCESS,
      data: {
        role_id: roleId,
        role_name: role.name,
  ...
  async getAllPermissions(): Promise<any> {
    ...
    const groupedPermissions = Object.entries(permissionGroups)
      .map(([groupKey, groupInfo]) => {
        const groupPerms = permissions.filter((perm) => {
          ...
          const matches = groupInfo.keywords.some((keyword) =>
            perm.name.includes(keyword),
          );
          if (matches) {
    ...
    const unassignedPerms = permissions.filter(
      (perm) => !assignedPermissions.has(perm.id),
    );
  ...
  async createRole(name: string, permissionIds?: number[]): Promise<any> {
    ...
    if (existingRole) {
      throw new BadRequestException(ResponseMessages.ROLE.ALREADY_EXISTS);
    }
    ...
    if (permissionIds && permissionIds.length > 0) {
      ...
      if (permissions.length !== permissionIds.length) {
        ...
        throw new BadRequestException(
          ResponseMessages.ROLE.PERMISSION_NOT_FOUND,
        );
      }
    ...
    if (!createdRole) {
      throw new NotFoundException(ResponseMessages.ROLE.FETCH_FAILED);
    }

    return {
      success: true,
      message: ResponseMessages.ROLE.CREATED_SUCCESS,
      data: {
        id: createdRole.id,
